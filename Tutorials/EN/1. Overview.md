...
### Mask
**Fluid Mask**, using an RGB texture as a mask to indicate which specific areas to show or rotate the fluid; 
...
**Albedo Mask**, referring to an RGB texture as a mask on which there can have 7 basic colors to indicate which areas on the Albedo to blend with additional colors;
...
> [!IMPORTANT]
> The bumps the normal masks manipulates is referring to ONLY Normal Map and its relevant properties, and NOT involving Detail Normal Map and its relevant properties, which means, the bumps produced by the latters receive no influence from the normal masks.
